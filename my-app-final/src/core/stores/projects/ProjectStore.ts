    ...
    persist(
      (set, get) => ({
        ...
        get currentWorkspaceProjects() {
          ...
          // TODO: When workspace functionality is implemented, filter by workspaceId
          // For now, use workspaceId as tenant ID (projects belong to tenant)
          return projects.filter(project => project.workspaceId === currentTenant.id);
        },

        get canCreateProject() {
          const currentTenant = useTenantStore.getState().currentTenant;
          const currentTenantRole = useTenantStore.getState().getCurrentUserRole();

          return !!(currentTenant && currentTenantRole && ['owner', 'admin', 'member'].includes(currentTenantRole));
        },
...
// TODO: Uncomment when workspace functionality is implemented
// let previousWorkspaceId: string | null = null;
//
// useTenantStore.subscribe((state: any) => {
