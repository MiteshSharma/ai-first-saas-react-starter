...
/**
 * Resource performance entry interface
 */
...
/**
 * Performance with memory property (Chrome-specific)
 */
...
export const initPerformanceMonitoring = (): void => {
  ...
  const sendToAnalytics = (metric: Metric) => {
    ...
    if (config.debug) {
      logger.info('Web Vital recorded', 'Performance', { name: metric.name, value: metric.value });
    }
...
export const monitorResourcePerformance = (): void => {
  ...
  const resources = window.performance.getEntriesByType('resource') as ResourcePerformanceEntry[];
  const slowResources = resources.filter((resource) => resource.duration > 1000);
  
  if (slowResources.length > 0) {
    logger.warn('Slow loading resources detected', 'Performance', slowResources);
    
    // Send slow resource alerts if needed
    slowResources.forEach((resource) => {
      if (process.env.REACT_APP_ENV === 'development') {
        logger.info(`Slow resource: ${resource.name} took ${resource.duration}ms`);
      }
    });
  }
};
 ...
 */
export const monitorMemoryUsage = (): MemoryInfo | null => {
  if ('memory' in window.performance) {
    ...
    if (process.env.REACT_APP_ENV === 'development') {
      logger.info('Memory usage', 'Performance', memoryInfo);
    }
...
export const observeLongTasks = (): void => {
  if ('PerformanceObserver' in window) {
    try {
      ...
      logger.error('Long task observer failed', 'Performance', error);
    }
  }
};
...
export const trackCustomMetric = (name: string, value: number, unit = 'ms'): void => {
  ...
  if (process.env.REACT_APP_ENV === 'development') {
    logger.info('Custom metric', 'Performance', metric);
  }
