...
import { AUTH_EVENTS, type AuthLoginPayload } from '../plugins/coreEvents';
import type {
  AuthState,
  AuthActions,
  LoginCredentials,
    ...
    (set, get) => ({
      ...
      login: async (credentials: LoginCredentials): Promise<void> => {
        ...
        try {
          ...
          eventBus.emit(AUTH_EVENTS.USER_LOGIN, loginPayload, 'AuthStore');
        } catch (error: unknown) {
      ...
      logout: async (): Promise<void> => {
        ...
        try {
          ...
          eventBus.emit(AUTH_EVENTS.USER_LOGOUT, {}, 'AuthStore');
        }
      },
