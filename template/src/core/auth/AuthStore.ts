...
export const useAuthStore = create<AuthStore>()(
  persist(
    (set, get) => ({
      ...
      ...createRequestLifecycleMethods(set as (partial: Partial<AuthStore>) => void),

      // Auth actions
      initializeAuth: (): void => {
        // Check if there's an existing authToken in localStorage
        const authData = getItem<{ userId: string; token: string; refreshToken: string }>('authToken');

        if (authData && authData.token) {
          // Parse the token to get user data (in a real app, you might decode JWT or make an API call)
          // For now, we'll check if auth-storage has persisted user data
          const persistedAuth = getItem<{ user: User; token: string }>('authToken');

          if (persistedAuth && persistedAuth.user && persistedAuth.token) {
            set({
      ...
      login: async (credentials: LoginCredentials): Promise<void> => {
        ...
        try {
          ...
          set({ user, token, isAuthenticated: true, loading: false, currentRequest: null, error: null });
